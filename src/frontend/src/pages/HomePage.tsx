...
function useStarfield(count: number): Star[] {
  const [stars] = useState<Star[]>(() => Array.from({ length: count }, () => ({
    top:     `${Math.random() * 100}%`,
...
// La mitad "Artistas" recibe una imagen aleatoria ya elegida por el padre (mismo
// enfoque que ArtistsPage.tsx: recopilar todas las artist_images de los artistas
// activos y elegir una al azar) — cambia en cada recarga, no persiste.
function ArtistsBolaSplitSection({ artistImageUrl }: { artistImageUrl: string | null }) {
  ...
  return (
    <section className="relative w-full flex flex-col md:flex-row">
      <SplitHalf
        ...
        backgroundImage={artistImageUrl}
      />
...
export default function HomePage() {
  ...
  // Imagen aleatoria para la mitad "Artistas" del split — Math.random() es un efecto
  // secundario (elección impura), no pertenece al cuerpo de un useMemo; se resuelve
  // una vez por montaje cuando `artists` pasa de [] al resultado real del fetch, y no
  // persiste entre recargas (no hay localStorage/sessionStorage de por medio).
  useEffect(() => {
    const allImageUrls = artists.flatMap(a => a.images.map(img => img.url))
    if (allImageUrls.length === 0) return
    setArtistImageUrl(allImageUrls[Math.floor(Math.random() * allImageUrls.length)])
  }, [artists])
  ...
  return (
    <div className="bg-canvas">
      ...
      <ArtistsBolaSplitSection artistImageUrl={artistImageUrl} />
